...
void TextureManager::initWithMaterialPaths(const std::unordered_set<std::string>& texturePaths)
{
    std::vector<std::string> filePaths(texturePaths.begin(), texturePaths.end());
    
    // Sort for consistent texture ID assignment
    std::sort(filePaths.begin(), filePaths.end());
    
    LoadTexturesFromPaths(filePaths);
}

/*
// Legacy init() method - replaced by initWithMaterialPaths() called from AssetRegistry
...
void TextureManager::LoadTexturesFromPaths(const std::vector<std::string>& filePaths)
{

    m_textures.resize(filePaths.size());
    textureNameIdLookup.clear();
    texturePathIdLookup.clear();
    ...
    for (int i = 0; i < filePaths.size(); ++i)
    {
        ...
        texturePathIdLookup[filePath] = i;
...
TexObj TextureManager::GetTextureByPath(const std::string& filepath)
{
    auto it = texturePathIdLookup.find(filepath);
    if (it == texturePathIdLookup.end())
    {
        printf("ERROR: Texture path '%s' not found\n", filepath.c_str());
        return 0;
    }
...
bool TextureManager::LoadTexture(const std::string& filepath)
{
    ...
    if (texturePathIdLookup.find(filepath) != texturePathIdLookup.end())
    {
        return true; // Already loaded
    }
    
    // Add to the texture collection and reload
    std::vector<std::string> currentPaths;
    for (const auto& pair : texturePathIdLookup)
    {
        currentPaths.push_back(pair.first);
    }
    currentPaths.push_back(filepath);
    
    LoadTexturesFromPaths(currentPaths);
    
    return texturePathIdLookup.find(filepath) != texturePathIdLookup.end();
}
