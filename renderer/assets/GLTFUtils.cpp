...
namespace GLTFUtils
{
    ...
    bool isGLTFFile(const std::string &filename)
    {
        if (filename.length() < 5)
            return false; // Need at least ".gltf" which is 5 characters
    ...
    bool extractAnimationsFromGLTF(std::vector<AnimationClip> &animationClips, const tinygltf::Model &model)
    {
        ...
        for (const auto &gltfAnimation : model.animations)
        {
            ...
            // Set looping behavior based on animation name/type
            // Place animations should not loop, others can loop by default
            if (clip.name.find("Place") != std::string::npos || clip.name.find("place") != std::string::npos)
            {
                clip.isLooping = false;
            }
            else
            {
                clip.isLooping = true;  // Default for other animations
            }
            
            animationClips.push_back(clip);
            std::cout << "Extracted animation: " << clip.name << " (duration: " << clip.duration << "s, looping: " << (clip.isLooping ? "yes" : "no") << ")" << std::endl;
        }
