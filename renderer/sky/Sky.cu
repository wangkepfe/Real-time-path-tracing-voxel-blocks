...
float SkyModel::getFittingData(const float* elevMatrix, float solarElevation, int i)
{
    return (powf(1.0f - solarElevation, 5.0f) * elevMatrix[i] +
        5.0f * powf(1.0f - solarElevation, 4.0f) * solarElevation * elevMatrix[i + 9] +
        10.0f * powf(1.0f - solarElevation, 3.0f) * powf(solarElevation, 2.0f) * elevMatrix[i + 18] +
        10.0f * powf(1.0f - solarElevation, 2.0f) * powf(solarElevation, 3.0f) * elevMatrix[i + 27] +
        5.0f * (1.0f - solarElevation) * powf(solarElevation, 4.0f) * elevMatrix[i + 36] +
...
float SkyModel::getFittingData2(const float* elevMatrix, float solarElevation)
{
    return (powf(1.0f - solarElevation, 5.0f) * elevMatrix[0] +
        5.0f * powf(1.0f - solarElevation, 4.0f) * solarElevation * elevMatrix[1] +
        10.0f * powf(1.0f - solarElevation, 3.0f) * powf(solarElevation, 2.0f) * elevMatrix[2] +
        10.0f * powf(1.0f - solarElevation, 2.0f) * powf(solarElevation, 3.0f) * elevMatrix[3] +
        5.0f * (1.0f - solarElevation) * powf(solarElevation, 4.0f) * elevMatrix[4] +
        powf(solarElevation, 5.0f) * elevMatrix[5]);
}

void SkyModel::initSkyConstantBuffer()
{
    CUDA_CHECK(cudaMemcpyToSymbol(cSolarDatasets, hSolarDatasets, sizeof(float) * 1800));
    CUDA_CHECK(cudaMemcpyToSymbol(cLimbDarkeningDatasets, hLimbDarkeningDatasets, sizeof(float) * 60));
}
...
void SkyModel::updateSkyState()
{
    ...
    float elevation = acos(1.0f - sunDir.y);
    ...
    for (channel = 0; channel < 10; ++channel)
    {
        for (int i = 0; i < 9; ++i)
        {
            hSkyConfigs[channel * 9 + i] = getFittingData(skyDataSets + channel * 54, solarElevation, i);
        }

        hSkyRadiances[channel] = getFittingData2(skyDataSetsRad + channel * 6, solarElevation);
    }

    CUDA_CHECK(cudaMemcpyToSymbol(cSkyConfigs, hSkyConfigs, sizeof(float) * 10 * 9));
    CUDA_CHECK(cudaMemcpyToSymbol(cSkyRadiances, hSkyRadiances, sizeof(float) * 10));
}
...
INL_DEVICE Float3 SpectrumToXyz(int channel)
{
    ...
    return Float3(spectrumCieX[channel], spectrumCieY[channel], spectrumCieZ[channel]) / CIE_Y_integral;
}

INL_DEVICE Float3 GetSkyRadiance(const Float3& raydir, const Float3& sunDir, SkyParams& skyParams)
{
...
INL_DEVICE Float3 GetSunRadiance(const Float3& raydir, const Float3& sunDir, SkyParams& skyParams)
{
    float theta = acos(raydir.y);
    float gamma = acos(clampf(dot(raydir, sunDir), -1, 1));
    ...
    const float sunAngle = 5.0f;
    const float solarRadius = sunAngle * M_PI / 180.0f / 2.0f;
    const float sunBrightnessScaleFactor = 1.0f / ((sunAngle / 0.51f) * (sunAngle / 0.51f));
...
#pragma unroll
    for (channel = 0; channel < 10; ++channel)
    {
        ...
        const float* coefs = cSolarDatasets + channel * 180 + (order * (pos + 1) - 1);
...
__global__ void SkySun(SurfObj sunBuffer, float* sunPdf, Int2 size, Float3 sunDir, SkyParams skyParams)
{
    ...
    const float sunAngle = 5.0f;
    Float3 raydir = EqualAreaMapCone(sunDir, u, v, cos(sunAngle * M_PI / 180.0f / 2.0f));
    ...
    Store2DHalf4(Float4(color, 0), sunBuffer, Int2(x, y));
...
void SkyModel::init()
{
    ...
    CUDA_CHECK(cudaMalloc((void**)&skyCdfScanTmp, skyScanBlockCount * sizeof(float)));
    ...
    CUDA_CHECK(cudaMalloc((void**)&sunCdfScanTmp, sunScanBlockCount * sizeof(float)));
...
void SkyModel::update()
{
    ...
    if (skyParams.needRegenerate)
    {
        ...
        // TODO: Compute sky resolution based on the camera resolution and FOV to reduce aliasing
        // const auto& renderer = OptixRenderer::Get();
        // const auto& camera = renderer.getCamera();
        // Float2 cameraRes = camera.resolution;
        ...
        // Generate sky on GPU
        Sky KERNEL_ARGS2(GetGridDim(skyRes.x, skyRes.y, BLOCK_DIM_8x8x1), GetBlockDim(BLOCK_DIM_8x8x1))
