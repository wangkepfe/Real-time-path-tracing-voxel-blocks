...
namespace jazzfusion
{
    ...
    // Returns normalized direction
    Float3 octToNdirUnorm32(unsigned int pUnorm)
    {
        Float2 p;
    ...
    // Unpack two 16-bit floats (packed into a single 32-bit uint)
    // into a Float2.
    __device__ Float2 Unpack_R16G16_FLOAT(unsigned int rg)
    {
    ...
    // Unpack two uint values (each containing two 16-bit floats)
    // into a Float4.
    ...
    __device__ Float4 Unpack_R16G16B16A16_FLOAT(UInt2 rgba)
    {
    ...
    __device__ inline unsigned int pack_f32_to_f16_bits(float f)
    {
    ...
    __device__ unsigned int Pack_R16G16_FLOAT(Float2 rg)
    {
    ...
    // Packs a Float4 into a UInt2 by packing each pair of floats into a 32-bit uint.
    // For example, rgba.rg is packed into the low uint and rgba.ba into the high uint.
    __device__ UInt2 Pack_R16G16B16A16_FLOAT(Float4 rgba)
    {
        unsigned int low = Pack_R16G16_FLOAT(make_float2(rgba.x, rgba.y));
        unsigned int high = Pack_R16G16_FLOAT(make_float2(rgba.z, rgba.w));
