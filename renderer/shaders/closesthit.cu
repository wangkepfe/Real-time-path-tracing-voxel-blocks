...
extern "C" __global__ void __closesthit__radiance()
{
    ...
    {
        // Skip local light sampling if no lights are available
        if (sysParam.numLights == 0 || !sysParam.lightAliasTable || !sysParam.lightAliasTable->initialized()) {
            break;
        }
        
        float sourcePdf;
        unsigned int lightIndex = sysParam.lightAliasTable->sample(rand(sysParam, randIdx), sourcePdf);
    ...
    for (unsigned int i = 0; i < numBrdfSamples; ++i)
    {
        ...
        if (GetSurfaceBrdfSample(surface, randIdx, sampleDir, brdfPdf, isTransmissiveEvent))
        {
            ...
            else
            {
                ...
                {
                    // Check if lightAliasTable is valid before accessing PMF
                    if (sysParam.lightAliasTable && sysParam.lightAliasTable->initialized()) {
