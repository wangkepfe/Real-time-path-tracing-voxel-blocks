...
namespace SelfIntersectionAvoidance
{
    ...
    __forceinline__ __device__ float __frcp_approx(const float f)
    {
        return __fdividef(1.f, f);
    }

    // 3D dot product
    __forceinline__ __device__ float dot_rn(const float3 &u, const float3 &v)
    {
        return __fmaf_rn(u.x, v.x, __fmaf_rn(u.y, v.y, __fmul_rn(u.z, v.z)));
    }

    __forceinline__ __device__ float dot_ru(const float3 &u, const float3 &v)
    {
        return __fmaf_ru(u.x, v.x, __fmaf_ru(u.y, v.y, __fmul_ru(u.z, v.z)));
    }

    __forceinline__ __device__ float dot_rd(const float3 &u, const float3 &v)
    {
        return __fmaf_rd(u.x, v.x, __fmaf_rd(u.y, v.y, __fmul_rd(u.z, v.z)));
    }

    __forceinline__ __device__ float dot_abs_rn(const float3 &u, const float3 &v)
    {
        return __fmaf_rn(fabsf(u.x), fabsf(v.x),
                         __fmaf_rn(fabsf(u.y), fabsf(v.y), __fmul_rn(fabsf(u.z), fabsf(v.z))));
    }

    __forceinline__ __device__ float dot_abs_ru(const float3 &u, const float3 &v)
    {
        return __fmaf_ru(fabsf(u.x), fabsf(v.x),
                         __fmaf_ru(fabsf(u.y), fabsf(v.y), __fmul_ru(fabsf(u.z), fabsf(v.z))));
    }

    __forceinline__ __device__ float length(const float3 &u)
    {
        return sqrtf(dot_rn(u, u));
    }

    __forceinline__ __device__ float3 normalize(const float3 &u)
    {
        const float s = rsqrtf(dot_rn(u, u));
        return make_float3(__fmul_rn(u.x, s), __fmul_rn(u.y, s), __fmul_rn(u.z, s));
    }

    __forceinline__ __device__ float length_L1(const float3 &u)
    {
        return __fadd_rn(__fadd_rn(fabsf(u.x), fabsf(u.y)), fabsf(u.z));
    }

    __forceinline__ __device__ float3 normalize_L1(const float3 &u)
    {
        const float s = __frcp_approx(length_L1(u));
        return make_float3(__fmul_rn(u.x, s), __fmul_rn(u.y, s), __fmul_rn(u.z, s));
    }

    __forceinline__ __device__ float __fmmsf_rn(const float a, const float b, const float c, const float d)
    {
        const float cd = c * d;
    ...
    __forceinline__ __device__ void offsetSpawnPoint(float3 &outPosition,
                                                     ...
                                                     const float offset)      // offset distance
    {
        ...
        outPosition.x = (direction.x > 0.f) ? __fmaf_ru(offset, direction.x, position.x) : __fmaf_rd(offset, direction.x, position.x);
        outPosition.y = (direction.y > 0.f) ? __fmaf_ru(offset, direction.y, position.y) : __fmaf_rd(offset, direction.y, position.y);
        outPosition.z = (direction.z > 0.f) ? __fmaf_ru(offset, direction.z, position.z) : __fmaf_rd(offset, direction.z, position.z);
    }

    // offset a position away from a surface along the surface normal
    __forceinline__ __device__ void offsetSpawnPoint(float3 &outFront,
                                                     float3 &outBack,
                                                     const float3 &position,  // position to offset
                                                     const float3 &direction, // offset direction
                                                     const float offset)      // offset distance
    {
        // offset point along direction, rounding away from the position
        outFront.x = (direction.x > 0.f) ? __fmaf_ru(offset, direction.x, position.x) : __fmaf_rd(offset, direction.x, position.x);
        outFront.y = (direction.y > 0.f) ? __fmaf_ru(offset, direction.y, position.y) : __fmaf_rd(offset, direction.y, position.y);
        outFront.z = (direction.z > 0.f) ? __fmaf_ru(offset, direction.z, position.z) : __fmaf_rd(offset, direction.z, position.z);

        outBack.x = (direction.x > 0.f) ? __fmaf_rd(-offset, direction.x, position.x) : __fmaf_ru(-offset, direction.x, position.x);
        outBack.y = (direction.y > 0.f) ? __fmaf_rd(-offset, direction.y, position.y) : __fmaf_ru(-offset, direction.y, position.y);
        outBack.z = (direction.z > 0.f) ? __fmaf_rd(-offset, direction.z, position.z) : __fmaf_ru(-offset, direction.z, position.z);
    }
    ...
    __forceinline__ __device__ float3 getTrianglePointAndError(float3 &outError,
    ...
    __forceinline__ __device__ float3 getObjectToWorldTransformedPositionAndError(float3 &outError, const Matrix3x4 &im, const float3 &p)
    {
    ...
    __forceinline__ __device__ float3 getWorldToObjectPreTranslatedTransformedPositionError(const Matrix3x4 &m,
    ...
    // error bounds on transformed position, plus a custom error term
    // the matrix is assumed to be exact
    __forceinline__ __device__ float3 getWorldToObjectTransformedPositionError(const Matrix3x4 &m,
                                                                               const float3 &p,         // source position
                                                                               const float3 &carry_err) // custom error term
    {
        constexpr float c0 = 1.19209317972490680404007434844970703125E-7f;
    ...
    __forceinline__ __device__ float3 transformDirection(const Matrix3x4 &m, const float3 &d)
    {
        float3 q;
    ...
    __forceinline__ __device__ float3 transposeTransformNormal(const Matrix3x4 &m, const float3 &n)
    {
        float3 q;
    ...
    __forceinline__ __device__ float3 rotateVectorByQuaternion(const float3 &v, const OptixSRTData &srt)
    {
    ...
    __forceinline__ __device__ float3 rotateVectorByQuaternionInversed(const float3 &v, const OptixSRTData &srt)
    {
        const float3 u = make_float3(srt.qx, srt.qy, srt.qz);
    ...
    __forceinline__ __device__ float3 getObjectToWorldTransformedPositionAndBiError(float3 &outError, const OptixSRTData &srt, float3 p)
    {
        // shear error
        const float px_err =
    ...
    __forceinline__ __device__ float3 transformDirection(const OptixSRTData &srt, float3 d)
    {
        // apply shear
    ...
    __forceinline__ __device__ float3 inverseTransposeTransformNormal(const OptixSRTData &srt, float3 n)
    {
    ...
    __forceinline__ __device__ Matrix3x4 invertMatrixPreTranslate(const Matrix3x4 &m)
    {
        const float det3 = m.row0.x * (m.row1.y * m.row2.z - m.row1.z * m.row2.y) - m.row0.y * (m.row1.x * m.row2.z - m.row1.z * m.row2.x) + m.row0.z * (m.row1.x * m.row2.y - m.row1.y * m.row2.x);
    ...
    __forceinline__ __device__ void optixGetSrtFromTransforms(OptixSRTData &srt, const Transform &transform, const float time)
    {
    ...
    template <typename Transform>
    __forceinline__ __device__ void optixGetMatrixFromTransforms(Matrix3x4 &o2w,
                                                                 Matrix3x4 &w2o,
...
#pragma unroll 1
    ...
    {
    public:
        __forceinline__ __device__ OptixTransform(OptixTraversableHandle handle)
            : m_handle(handle) {};

        __forceinline__ __device__ OptixTransformType getTransformTypeFromHandle() const
        {
            return optixGetTransformTypeFromHandle(m_handle);
        }

        __forceinline__ __device__ const OptixSRTMotionTransform *getSRTMotionTransformFromHandle() const
        {
            return optixGetSRTMotionTransformFromHandle(m_handle);
        }

        __forceinline__ __device__ const OptixMatrixMotionTransform *getMatrixMotionTransformFromHandle() const
        {
            return optixGetMatrixMotionTransformFromHandle(m_handle);
        }

        __forceinline__ __device__ const OptixStaticTransform *getStaticTransformFromHandle() const
        {
            return optixGetStaticTransformFromHandle(m_handle);
        }

        __forceinline__ __device__ const Matrix3x4 getInstanceTransformFromHandle() const
        {
            return optix_impl::optixLoadReadOnlyAlign16(reinterpret_cast<const Matrix3x4 *>(optixGetInstanceTransformFromHandle(m_handle)));
        }

        __forceinline__ __device__ const Matrix3x4 getInstanceInverseTransformFromHandle() const
        {
    ...
    public:
        ...
        __forceinline__ __device__ OptixTransform getTransform(unsigned int index) const
        {
    ...
    public:
        ...
        __forceinline__ __device__ OptixTransformList(int size, const OptixTraversableHandle *handles)
            : m_size(size), m_handles(handles) {};

        __forceinline__ __device__ unsigned int getTransformListSize() const { return m_size; }
