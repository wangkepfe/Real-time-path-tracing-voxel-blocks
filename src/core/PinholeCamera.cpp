...
bool PinholeCamera::getFrustum(float3& pos, float3& u, float3& v, float3& w)
{
  ...
  if (changed)
  {
    ...
    float3 normal = make_float3(cosPhi * sinTheta, -cosTheta, -sinPhi * sinTheta); // "normal", unit vector from origin to spherical coordinates (phi, theta)

    float tanFov = tanf((m_fov * 0.5f) * M_PIf / 180.0f); // m_fov is in the range [1.0f, 179.0f].
    m_cameraPosition = m_center + m_distance * normal;

    m_cameraU = m_aspect * make_float3(-sinPhi, 0.0f, -cosPhi) * tanFov;               // "tangent"
    m_cameraV = make_float3(cosTheta * cosPhi, sinTheta, cosTheta * -sinPhi) * tanFov; // "bitangent"
